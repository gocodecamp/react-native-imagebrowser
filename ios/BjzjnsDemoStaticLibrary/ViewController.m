...
-(instancetype)initWithThumbImagesUrls:(NSArray *)thumbImagesUrls BigImagesUrls:(NSArray *)bigImagesUrls{
  if (self = [super init]) {
    self.bigImagesUrls = [NSMutableArray arrayWithArray:bigImagesUrls];
    self.thumbImagesUrls = [NSMutableArray arrayWithArray:thumbImagesUrls];
  }
  return self;
}
...
- (void)imgTaped:(UITapGestureRecognizer *)sender{
    ...
    [photoGallery tappedImageView:(UIImageView *)sender.view andImageUrls:self.bigImagesUrls andInitialIndex:[self indexForView:sender.view]];
...
- (NSInteger)indexForView:(UIView *)view{
  ...
  CGFloat width = PADDING + IMAGE_SIZE;
  CGFloat Y = self.view.center.y-IMAGE_SIZE-10;
  
  int i = view.frame.origin.x / width + 1;
